...
That's it. The package ships a self-contained `editor.css` with all styling pre-compiled (utility classes + component styles + WYSIWYG typography). You do **not** need Tailwind CSS or `@tailwindcss/typography` installed in your project — just import the CSS file once (see [Usage](#usage) below).
