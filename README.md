...
2. **Inline utility classes in templates** (e.g. `flex`, `items-center`, `min-w-[300px]`) — Tailwind's content scanner cannot see files inside `node_modules` (they are excluded via `.gitignore`), so these classes won't be generated by your Tailwind build. The package ships a pre-compiled CSS file (`dist/editor.css`) that contains all of them. **You must import it once:**

```js
import 'svelte-wysiwyg-editor/editor.css';
```

The editor area uses `@tailwindcss/typography` (`prose` classes) for WYSIWYG content styling. The `editor.css` file does **not** include `prose` styles to avoid conflicting with your project's own typography theme. You must have `@tailwindcss/typography` installed and configured in your project, and ensure `prose` is generated (either by using it somewhere in your own source, or by adding it to your Tailwind `safelist`):

```js
// tailwind.config.js
module.exports = {
  // ...
  safelist: ['prose', 'prose-sm', 'prose-lg', 'prose-invert'],
